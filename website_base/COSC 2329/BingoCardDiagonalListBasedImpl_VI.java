...
public class BingoCardDiagonalListBasedImpl_Lewis extends BingoCardDiagonalListBased_Abstract {


	public BingoCardDiagonalListBasedImpl_Lewis(Mystery_D mystery)
	{
		super(mystery);
	}
	public BingoCardDiagonalListBasedImpl_Lewis(List<List<Integer>> list)
	{
		diagonalListList = list;
	...
	public Integer getEntry(int row, int column) {
		...
		else
		{
			...
			int diagonalRowNum = 4 - difference;
			List<Integer> myRow = this.diagonalListList.get(diagonalRowNum);
			for(int i = 0; i < myRow.size(); i++)
			{
				int myInt = myRow.get(i);
