...
public class BingoCardColumnListBasedImpl_Lewis extends BingoCardColumnListBased_Abstract {

	public BingoCardColumnListBasedImpl_Lewis(Mystery_C mystery)
	{
		super(mystery);
	}
	public BingoCardColumnListBasedImpl_Lewis(List<List<Integer>> list)
	{
		columnListList = list;
	...
	public Integer getEntry(int row, int column) {
		...
		Integer entry = this.columnListList.get(column-1).get(row-1); // just switch column and row and subtract 1
		if(entry != null)
		{
			// a check for non-null values to make sure it is in range
			assert ((column - 1)*15 + 1) <= entry && entry <= ((column - 1) + 1)*15 : "ERROR: entry = " + entry + " out of range!";
		}
