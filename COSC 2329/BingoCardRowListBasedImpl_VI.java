...
public class BingoCardRowListBasedImpl_Lewis  extends BingoCardRowListBased_Abstract {

	public BingoCardRowListBasedImpl_Lewis(Mystery_B mystery)
	{
		super(mystery);
	}
	public BingoCardRowListBasedImpl_Lewis(List<List<Integer>> list)
	{
		//assert isValidBingoCard(list): "ERROR: Invalid Bingo Card!";
		rowListList = list;
		integersMarked = new HashSet<Integer>();
		integersMarked.add(FREE_SPACE);
	}

	public boolean isValidBingoCard(List<List<Integer>> rowListList)
	{
		boolean isValid = true;
		int column = 0;
		while(isValid && column < COLUMN_COUNT)
		{
			int row = 0;
			while(isValid && row < ROW_COUNT)
			{
				List<Integer> currRow = rowListList.get(row);
				Integer currEntry = currRow.get(column);
				if(currEntry <= ((column)*15 + 1) || currEntry >= ((column) + 1)*15)
				{
					isValid = false;
				}
			}
		...
		int currentCol = 0;
		int currentRow = 0;
		while(isValid && currentCol < COLUMN_COUNT)
		{
			while(isValid && currentRow < ROW_COUNT)
			{
				Integer currEntry = rowListList.get(currentRow).get(currentCol);
				int row = 0;
				while(isValid && row < ROW_COUNT)
				{
					Integer compareEntry = rowListList.get(row).get(currentCol);
					if(currEntry == compareEntry)
					{
						isValid = false;
					}
					row++;
	...
	public Integer getEntry(int row, int column) {
		...
		Integer retInt;
		if(row != BingoCard.FREE_SPACE_ROW)
		{
			retInt = this.rowListList.get(row-1).get(column-1);
		}
		else {
			if(column == BingoCard.FREE_SPACE_COLUMN)
			{
				retInt = BingoCard.FREE_SPACE;
			}
			else if(column > BingoCard.FREE_SPACE_COLUMN)
			{
				retInt = this.rowListList.get(row-1).get(column-2);
			}
			else
			{
				retInt = this.rowListList.get(row-1).get(column-1);
	...
	private boolean is5InARow(int rowStart, int colStart, int deltaRow, int deltaCol)
	{
		...
		while(row <= BingoCard.ROW_COUNT && col <= BingoCard.COLUMN_COUNT)
		{
			if(!isMarked(row, col))
			{
				winner = false;
